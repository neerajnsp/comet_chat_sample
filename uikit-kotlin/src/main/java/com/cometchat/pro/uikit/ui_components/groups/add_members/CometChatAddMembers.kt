...
import com.cometchat.pro.uikit.ui_components.shared.cometchatUsers.AddMemberAdapter
...
class CometChatAddMembers : Fragment() ,AddChildInterface{
    private var userListAdapter: AddMemberAdapter? = null
...
//        }))
    ...
    private fun setAdapter(users: List<User>) {
        if (userListAdapter == null) {
            userListAdapter = AddMemberAdapter(context!!,this)
    ...
    override fun remove(uid: String) {
            userList.remove(GroupMember(uid, CometChatConstants.SCOPE_PARTICIPANT))
if(userList.size===0){
    ivAdd!!.visibility = View.GONE
}
    }
}
interface AddChildInterface{
    fun add(uid:String);
